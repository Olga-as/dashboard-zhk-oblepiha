...
// Компактный формат крупных чисел: 700 000 -> "700 к", 271 472 771 -> "271 млн"
...
const app = createApp({
    ...
    computed: {
        ...
        floorOptions() {
            return [...new Set(this.apartments.map(a => a.floor))].sort((a, b) => a - b);
        },
        roomOptions() {
            return [...new Set(this.apartments.map(a => a.rooms))].sort((a, b) => a - b);
        },
        planOptions() {
            return [...new Set(this.apartments.map(a => a.plan).filter(Boolean))].sort();
        },
        areaOptions() {
            return [...new Set(this.apartments.map(a => areaBucket(a.area)))].sort();
        },
        viewOptions() {
            return [...new Set(this.apartments.map(a => a.view).filter(Boolean))].sort();
        },
        ...
        filteredApartments() {
            let list = this.paramFilteredApartments;
            if (this.searchQuery) {
    ...
    methods: {
        ...
        renderMainChart() {
            ...
            const width = 1420;
            const height = 660;
            const margin = { top: 70, right: 5, bottom: 130, left: 110 };
        ...
        renderManagerSideChart() {
            ...
            const all = this.paramFilteredApartments;
            // Учитываем только квартиры, подходящие под выбранные фильтры (не otherParam)
            const sold = all.filter(a => a.sold && a.manager && !a.otherParam);
            ...
            svg.append('g').attr('transform', `translate(${margin.left},0)`).call(d3.axisLeft(y).ticks(5).tickFormat(d => (isDiscount || isPrice) ? formatCompact(d) : d));
            ...
            svg.selectAll('.label')
                ...
                .style('font-size', '11px')
                .style('font-weight', 'bold')
                .text(d => (isDiscount || isPrice) ? formatCompact(d[metric]) : d[metric]);

            // Фактоид сверху: зависит от выбранного режима «Продажи»
            // Исключаем квартиры, не подходящие под фильтры (otherParam) и проданные в другом месяце (otherMonth)
            const filtered = all.filter(a => !a.otherParam && !a.otherMonth);
            ...
            if (this.colorMode === 'status') {
                // По количеству: сколько квартир продано из скольки
                const total = filtered.length;
                const soldCount = filtered.filter(a => a.sold).length;
                ...
                // По скорости продаж: средняя скорость продаж вообще
                const soldAll = filtered.filter(a => a.sold);
                const avgDays = soldAll.length ? Math.round(soldAll.reduce((s, a) => s + (a.days || 0), 0) / soldAll.length) : 0;
                factoid.innerHTML = `<div style="font-size:22px;font-weight:bold;color:#1a2980">${avgDays} <span style="color:#999;font-weight:normal;font-size:14px">дней</span></div>
                    <div style="font-size:12px;color:#666">средняя скорость продаж</div>`;
            } else if (this.colorMode === 'price') {
                // По цене продажи: сумма выручки по всем менеджерам (за выбранный период)
                const revenue = filtered.reduce((s, a) => s + (a.priceSale || 0), 0);
                factoid.innerHTML = `<div style="font-size:22px;font-weight:bold;color:#1a2980">${formatCompact(revenue)}</div>
                    <div style="font-size:12px;color:#666">выручка по всем менеджерам</div>`;
            } else if (this.colorMode === 'discount') {
                // По скидке: сумма скидок по всем менеджерам (за выбранный период)
                const discount = filtered.reduce((s, a) => s + (a.discount || 0), 0);
                factoid.innerHTML = `<div style="font-size:22px;font-weight:bold;color:#1a2980">${formatCompact(discount)}</div>
                    <div style="font-size:12px;color:#666">скидки по всем менеджерам</div>`;
            }

            if (factoid.innerHTML) {
                container.insertBefore(factoid, container.firstChild);
            }
        },

        // Разбивка продаж по параметрам: горизонтальные «стеки» (продано/не продано)
        renderSalesBreakdown() {
            const container = document.getElementById('salesBreakdown');
            if (!container) return;
            container.innerHTML = '';

            // Используем отфильтрованные квартиры (без otherParam — не подходящих под фильтры)
            const list = this.paramFilteredApartments.filter(a => !a.otherParam);

            // Определения измерений: [заголовок, функция получения значения]
            const dimensions = [
                { title: 'Корпус', get: a => a.corp },
                { title: 'Этаж', get: a => String(a.floor) },
                { title: 'Комнат', get: a => a.rooms + ' комн.' },
                { title: 'Планировка', get: a => a.plan || '—' },
                { title: 'Площадь', get: a => areaBucket(a.area) },
                { title: 'Цена', get: a => priceBucket(a.price) },
                { title: 'Вид из окна', get: a => a.view || '—' }
            ];
            ...
            dimensions.forEach(dim => {
                ...
                list.forEach(a => {
                    const key = dim.get(a);
                    if (!groups[key]) groups[key] = { label: key, sold: 0, total: 0 };
                    groups[key].total++;
                    if (a.sold) groups[key].sold++;
                });

                let data = Object.values(groups);
                if (data.length === 0) return;

                // Сортируем: сначала лучше продающиеся (по доле продаж), затем по количеству
                data.sort((a, b) => (b.sold / b.total) - (a.sold / a.total) || b.total - a.total);
                ...
                header.className = 'breakdown-title';
                header.textContent = dim.title;
                ...
                data.forEach(d => {
                    ...
                    const pct = d.total ? Math.round(d.sold / d.total * 100) : 0;
                    const soldW = d.total ? (d.sold / maxTotal) * 100 : 0;
                    const unsoldW = d.total ? ((d.total - d.sold) / maxTotal) * 100 : 0;

                    const soldSeg = document.createElement('div');
                    soldSeg.className = 'breakdown-seg sold';
                    soldSeg.style.width = soldW + '%';
                    soldSeg.title = `Продано: ${d.sold} из ${d.total}`;

                    const unsoldSeg = document.createElement('div');
                    unsoldSeg.className = 'breakdown-seg unsold';
                    unsoldSeg.style.width = unsoldW + '%';
                    unsoldSeg.title = `Не продано: ${d.total - d.sold} из ${d.total}`;

                    barWrap.appendChild(soldSeg);
                    barWrap.appendChild(unsoldSeg);
                    row.appendChild(barWrap);

                    const val = document.createElement('div');
                    val.className = 'breakdown-value';
                    val.textContent = `${d.sold}/${d.total} (${pct}%)`;
                    row.appendChild(val);
